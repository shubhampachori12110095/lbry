...
class Daemon(AuthJSONRPCServer):
    ...
    def get_est_cost_from_uri(self, uri):
        ...
        resolved = (yield self.wallet_manager.resolve(uri))[uri]
        if resolved:
    ...
    def jsonrpc_block_show(self, blockhash=None, height=None):
        ...
        if blockhash is not None:
            d = self.wallet_manager.get_block(blockhash)
        elif height is not None:
            d = self.wallet_manager.get_block_info(height)
            d.addCallback(lambda b: self.wallet_manager.get_block(b))
        else:
